					...
					with oring.begin_sequence(time_tag=base_time_tag, header=ohdr_str) as oseq:
						for ispan in iseq_spans:
							...
							with oseq.reserve(ogulp_size) as ospan:
								...
								BFMap("a(i,j) *= 1./16.*exp(Complex<float>(0.0, -2*BF_PI_F*fmod(g(0)*s(0), 1.0)))*b(i)", {'a':gdata, 'b':self.phaseRot, 'g':self.phaseState, 's':self.sampleCount}, axis_names=('i','j'), shape=gdata.shape, extra_code="#define BF_PI_F 3.141592654f")
								gdata = gdata.reshape((-1,nstand,npol))
